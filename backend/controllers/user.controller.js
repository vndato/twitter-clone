...
export const followUnfollowUser = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const userToModfiy = await User.findById(id);
        const currentUser = await User.findById(req.user._id);

        if (id === req.user._id.toString()) return res.status(400).json({ error: "You can't follow/unfollow yourself" });

        if (!userToModfiy || !currentUser) return res.status(404).json({ error: "User not found" });
        ...
        if (isFollowing) {
            ...
            // TOO: return the id of the user as a response
